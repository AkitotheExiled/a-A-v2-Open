...
console.log(x[0] === y[0]) // true
 ...
 if (Array.isArray(array)) {
    ...
    dupe.push(deepDup(removed, dupe));
    dupe.push([deepDup(array, dupe)]);
    return dupe;
...
function deepDup(array,) {
  ...
  if (Array.isArray(array)) {
    ...
    // Contains items? Remove one of those items
    let shifted = array[0];
    // We need to figure out if the unidentified item we took out is another box or an item
    let removed = deepDup(shifted);
    let otherWork = deepDup(array.slice(1));
    // We also need to figure out if the remaining unidentified items are boxes or other items
    otherWork.unshift(removed);
    // return everything we found in the order we found it
    return otherWork;
  } else {
    // not a box? must be the item inside it
    return array;
  }
}
let arr = [1, [2, [3]]];
